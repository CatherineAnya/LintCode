...
############
# 题目分析：
# 一般算法：使用两个循环来把两个数组的值添加到新的数组中，然后使用sort()方法排序新的数组，时间复杂度为O(NlogN)
# 优化算法：因为两个数组都是排好序的，只需通过一个循环和两个偏移量比较两个数组的元素大小，不需要使用sort()方法，时间复杂度为O(N)
############
# 知识点：
# 1. list.sort()：Python列表的方法，对原列表进行排序，无返回值，直接更新列表，默认使用快速排序的方法以升序的形式排序，时间复杂度为O(NlogN)
# 2. list.extend()：Python列表的方法，在列表末尾一次性追加另一个序列中的多个值，无返回值，直接更新列表，参数为一个可迭代的对象
# 3. list.extend(L)等价于list[len(list):] = L
############
# 一般算法：
class Solution:
    """
